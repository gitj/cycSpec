...
gpubasedir = '/lakitu/data/P2721'
plotsdir = '/home/gjones/plots'
...
def plotDynSpecForScan(sn,mjd):
    import psrchive
    files = glob.glob(os.path.join(gpubasedir,('/gpu*/rt/rtcs_%d*_%04d_*%04d.ar.fix' % (mjd,sn,1))))
    if len(files) == 0:
    ...
    on,off,frqs,times = getDynSpec(os.path.join(gpubasedir,('gpu*/rt/rtcs_%d*_%04d_' % (mjd,sn))+ '*%04d.ar.fix'), band, onp, offp)
    ...
    fname = os.path.join(plotsdir,(os.path.split(files[0])[1] + '_'+ 'dynspec.png'))
    print fname
