...
try:
    ...
    import site
    site.addsitedir('/home/gpu/gjones/lib/python')
    from katcp_wrapper import FpgaClient
try:
...
#from osf_utils import segFreq, getObsFreqs

boffile = 'osf8rlo2ddc8t9fx16ka105_2013_Apr_08_1133.bof'
numberOfBofRegisters = 53  # len(r1.listdev()) for this boffile
...
try:
    ...
    raise Exception("osf.py - roach not found!")
    r1 = None    
gpud = {}

gpus = [1,2,3,5,6,7,8,9]

def getServers(glist=gpus):
    global gpud
    for k in glist:
        if not globals().has_key('g%d' % k):
            print "getting server", k
            proxy = Pyro4.Proxy("PYRONAME:guppiServer%d" % k)
...
def startDspsr(gpus=gpus,cal = False,scanlen=60.0,countdown=True,scannum = None, **kwargs):
    """
    kwargs passed to dspsr:
    nthread=4,nfft=32768,ncyclic=512,nbin=512, lsubint=10.0
    """
    sn = updateScanNum(gpus,scannum=scannum)
    calstr = "OFF"
    if cal:
        calstr = "ON"
    t = int(time.time())
    print "stopping any previous observation"
    killAll(gpus)
    print "syncing to get data flowing"
    sync()
    startat = t + 10 # startat is even pps tick
    armat = startat-0.8 # armat is when we arm the roach
    print "initializing scan %d planning to start at %d" % (sn,startat)
    setStartTime(startat, gpus) 
    for gpu in gpus:
        g = gpud[gpu]
        g.setParams(OBS_MODE="ONLYNET",
                    CAL_MODE=calstr,
                    SCANLEN=scanlen,
                    )
        g.startProc(buildDspsrCommand(gpu, **kwargs))
        g.csCommand("START")
    time.sleep(2)
    for gpu in gpus:
        g = gpud[gpu]
    ...
    if countdown:
        ...
        while time.time() < endat:
            ...
            for gpu in gpus:
                g = gpud[gpu]
                d = g.getParam("DROPTOT")["DROPTOT"]
                if d > 0.01:
                    drops[gpu] = d
                ns = g.getParam("NETSTAT")["NETSTAT"]
                if ns == "blocked":
                    blocked.append(gpu)
                p = g.pollProc()
                if p is not None:
                    returns[gpu] = p
            print ("\r%7.1f s..." % (endat - time.time())),
            if drops:
                print " * dropped packets:",str(drops),
            if blocked:
                print " * blocked:",str(blocked),
            if returns:
                print " * dspsr died:",str(returns),
            sys.stdout.flush()
            time.sleep(0.1)
        print "stopping"
        killAll(gpus)
...
def startRawCal(gpus=gpus,scanlen=60.0):
    startRaw(gpus=gpus,cal="ON",scanlen=scanlen)
    
def startRaw(gpus=gpus,cal = "OFF",scanlen=60.0):
    sn = updateScanNum(gpus)
    t = int(time.time())
    startat = t + 7 # startat is even pps tick
...
def setEq(addr,gain,pol=None):
    r1.write_int('pol1_addr',0)
    if pol is None or pol == 0:
        r1.write_int('pol0_addr',0)
        r1.write_int('pol0_gain',gain)
        r1.write_int('pol0_addr',addr)
        r1.write_int('pol0_addr',0)
    if pol is None or pol == 1:
        r1.write_int('pol1_addr',0)
        r1.write_int('pol1_gain',gain)
        r1.write_int('pol1_addr',addr)
        r1.write_int('pol1_addr',0)
...
def checkRoach(nreg = numberOfBofRegisters, clk = 128.0):
    error = ''
    try:
        actual_nreg = len(r1.listdev())
        if actual_nreg == nreg:
            meas_clk = r1.est_brd_clk()
            if np.abs(clk - meas_clk) < 2.0:
                return True
            else:
                error = "FPGA clock was %.1f MHz, expected %.1f MHz\n" % (meas_clk,clk)
        else:
            error += ("FPGA has %d registers, expected %d registers" % (actual_nreg, nreg))
    except Exception,e:
        raise Exception("Error accessing the ROACH. Error was: " + str(e))
...
def setAdcClock(frq,extref=True,chan_spacing=2.0):
    ...
    if reffreq != 10*1000*1000:
        raise Exception("Error! frequency reference is not 10 MHz!!")
        vs.conn.close()
        return
...
def plotGpu(gpun,Fs=1024.,rf=True,ndat=2**23, x16=True, db=True):
    gpu = gpud[gpun]
    ...
    nyq = nyqdict[band]
    if rf:
        fc = getObsFreqs(s, band, x16=x16, Fs=Fs)
    else:
...
#    d = rawData(gpu.getData(0,ndat))
    ...
    for ch in range(d.shape[1]):
        ...
        if db:
            ...
            ax2.plot(sign*freqs + fc[ch], (Pxx),'b')
            ax2.plot(sign*freqs + fc[ch], (Pyy),'r')

    
def plotAdc(Fs=1024.0, lo=-2, x16=True):
    x,y = getAdc()
    f = plt.figure()
    ...
    for r in range(4):
        ...
        if x16:
            if r < 2:
                gpufr = fsh[r*nf/8:(r+1)*nf/8]
            else:
                gpufr = fsh[(r+4)*nf/8:(r+5)*nf/8]
        else:
            gpufr = fsh[r*nf/4:(r+1)*nf/4]
        if r % 2:
            color = 'g'
        else:
            color = 'm'
        rect = plt.Rectangle((gpufr[0],0),(gpufr[-1]-gpufr[0]),1, transform=trans, alpha=0.2, color=color)
        ax2.add_patch(rect)
        print (fr[-1]-fr[0])/2.0
        print ("%d : E %d  O %d" % (r,gpua,gpub))
        ax2.text((gpufr[-1]-gpufr[0])/2.0 + gpufr[0],0.87,("%d : E %d  O %d" % (r,gpua,gpub)),transform=trans,fontdict=dict(size='small'),ha='center',va='bottom')
        ax2b.plot(fr[::2],20*np.log10(np.abs(d[:,r*2,0,0])),'r')
        ax2b.plot(fr[1::2],20*np.log10(np.abs(d[:,r*2+1,0,0])),'b')
        rect = plt.Rectangle((gpufr[0],0),(gpufr[-1]-gpufr[0]),1, transform=transb, alpha=0.2, color=color)
        ax2b.add_patch(rect)
...
def buildDspsrCommand(gpu,nthread=4,nfft=32768,ncyclic=512,nbin=256, lsubint=10.0,extra=""):
    g = gpud[gpu]
    params = g.getParam() #get all current parameters
    cal = params['CAL_MODE']
    calstr = ""
    if cal != "OFF":
        calstr = "_cal"
    fname = ('/data/gpu/partial/gpu0%(gpu)d/cspuppi/rt/rtcs_%(mjd)d_%(source)s_%(scan)s_%(band)s_nth%(nthread)d%(cal)s' %
             dict(gpu = gpu, mjd = params['STT_IMJD'], source= params['SRC_NAME'], scan = ('%04d' % params['SCANNUM']), band = params['FRONTEND'],
                  nthread=nthread,cal=calstr))
    args = ['-O', fname,
            '-d2',
            '-overlap',
            ('-F%d:D' % params['OBSNCHAN']),
            '-b', nbin,
            '-L', lsubint,
            '-cuda', (','.join(['0']*nthread)),
            '-T', params['SCANLEN'],
    ...
    if ncyclic:
        args = args + ['-cyclic', ncyclic]
    if cal != "OFF":
        args = args + ['-c 0.04 -D0.0001 -j "e type=PolnCal"']
    else:
        args = args + ['-E', params['PARFILE']]
    args = args + ['-header INSTRUMENT guppi_daq DATABUF 1']
    args = map(str,args)
    cmd = 'source /home/gpu/gjones/puppi.sh; dspsr ' + ' '.join(args)
    return cmd
    pass   
        
def killAll(gpus=gpus):
    print "stopping real time cyclic spectroscopy daqs"
    sys.stdout.flush()
    for gpu in gpus:
        g = gpud[gpu]
        g.endProc()
...
def setupObs(parfile, band, gpus = gpus, Fs=1024.0, nchan=32, projid='P2721'):
    if nchan == 32:
        x16 = True
    else:
        x16 = False
    ...
    for gpu,seg in zip(gpus,segs):
        ...
        g = gpud[gpu]
        frqs = getObsFreqs(seg, band, x16=x16, Fs=Fs)
        fc = frqs[nchan/2]
        chanbw = frqs[1]-frqs[0] 
        bw = chanbw * nchan
...
def autoLevel(gpus=gpus, goal=20.0,nchan=512,init_gain=128,x16=True,pass2=True):
    stopData()
    time.sleep(1)
...
def updateScanNum(gpus=gpus, scannum = None):
    if scannum is None:        
        try:
            g = gpud[gpus[0]]
