    ...
    def restartDaq(self):
        ...
        if self.daqp:
            ...
            if self.daqp.poll() is None:  #if it's still running
                ...
                os.killpg(self.daqp.pid,signal.SIGKILL)
        try:
    ...
    def startProc(self,cmd):
        ...
        if self.subp:
            try:
                ...
                logging.exception("exception in killing subprocess")
                killed = False
        else:
            killed = False
#        self.subp = subprocess.Popen(('%s' % (cmd,)).split(' '))
        self.subp = subprocess.Popen(('%s &> /home/gpu/gjones/logs/dspsr%d.log' % (cmd,gpu)),shell=True, preexec_fn=os.setsid)
        return killed
    ...
    def sendSig(self,sig):
        if self.subp:
            ...
            try:
                logging.info("Sending signal to pid %d",self.subp.pid)
                os.killpg(self.subp.pid,sig)
                return True
            except:
                return False
        else:
